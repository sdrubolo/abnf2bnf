...
object BnfSimplify {
  ...
  private def updateStore(replaceName:String,store:Set[String]):Set[String]
    = if (store(replaceName)) store - replaceName else store + replaceName
  ...
  private def canReplace(replace:Alternatives,alternatives:Alternatives,cons:Cons) : Boolean = {
    ...
    isSingleRule || isSingleWithMultiple || isSingle(cons)
  }
      ...
      case (acc,(ruleName,alternatives)) =>
        alternatives.foldLeft(acc) {
          case (acc,cons) => cons.foldLeft(acc) {
            case (acc,BnfName(name)) if isInternalRule(name) && canReplace(terminalRule(name,rules),alternatives,cons) =>
              updateStore(name,acc)
            case (acc,_) => acc
          }
        }
    }
